...
def quantize_fixed16_tensor(x_tensor):
    """Quantizes a PyTorch Float32 tensor to 16-bit signed fixed-point (Q12.4 format)."""
    x_scaled = torch.round(x_tensor * 16.0).clamp(-32768.0, 32767.0)
    return x_scaled / 16.0

class Calibrated16BitFixedConv2d(nn.Conv2d):
    """Simulates 16-Bit Fixed-Point Hardware execution (Q12.4 format)."""
...
def run_16bit_resnet_eval():
    ...
    print(f"=================================================================")
    print(f" 16-Bit Fixed-Point (Q12.4) Accuracy Evaluation (`16bittest_q12_4`)")
