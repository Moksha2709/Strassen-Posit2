# =============================================================================
# verify_16bit_resnet_hardware.py — Hardware-in-the-Loop ResNet-50 Benchmark (Q12.4)
# -----------------------------------------------------------------------------
# Evaluates PyTorch ResNet-50 layers via 100% full RTL hardware execution (Icarus Verilog vvp)
# using 16x16 Strassen GEMM tiles (Q12.4 format).
# =============================================================================
import os
import sys
import math
...
def encode_fixed16(x):
    """Encodes float32 value into signed 16-bit fixed-point (Q12.4 format)."""
...
def decode_fixed16(b):
    """Decodes signed 16-bit integer into float32 value (Q12.4 format)."""
    if b >= 32768:
        val = b - 65536
    else:
        val = b
...
def float_row_to_hex(row_vals):
    """Formats 8 float values into a 128-bit hex string (8 x 16-bit words)."""
    words = [encode_fixed16(x) for x in row_vals]
    return "".join(f"{w:04x}" for w in reversed(words))

def parse_hex_row(hex_str):
    """Parses a 128-bit hex string into 8 float values."""
...
def main(weights="../test8bit/weights_resnet50_cifar10_seed42_fp32.pth", dataset="cifar10", layer_indices=None):
    print("=" * 90)
    print("  Full Hardware-in-the-Loop ResNet-50 Benchmark: 16-Bit Fixed-Point (Q12.4)")
    print("=" * 90)
    ...
    print("\n" + "=" * 90)
    print("Verified Status   : SUCCESS (Full Hardware-in-the-Loop Q12.4 Validation Complete)")
    print("=" * 90 + "\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full Hardware-in-the-Loop ResNet-50 Benchmark (Q12.4)")
