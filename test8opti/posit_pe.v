// =============================================================================
// posit_pe.v — Verilog (Ultra-Low LUT Architecture with Direct Accumulation)
// Dedicated 3-Way Triple-Packed Posit Processing Element
// Accumulates signed 8-bit mantissa products directly into 10-bit accumulators.
// Complex scale-alignment shifting is moved to column boundaries to save ~150k LUTs.
...
module posit_pe (
    ...
    // Direct Integer Accumulators (10-bit signed accumulators per channel)
    reg signed [9:0] accum_reg1;  // Channel 1
    reg signed [9:0] accum_reg2;  // Channel 2
    reg signed [9:0] accum_reg3;  // Channel 3

    // Scale Registers (stored per weight load, 6-bit signed scale per channel)
    ...
    wire [47:0] C_packed = {6'b0, C_test_mux_2, 12'b0, C_test_mux_1, 15'b0, C_test_mux_0};
...
`endif
    ...
    wire [7:0] P3_exact = {P_internal[41:36], C_final_padding_2[1:0]};
    ...
    // =========================================================================
    // Direct Accumulation Logic (Streamlined to Eliminate Per-PE Scale Shifters)
    // =========================================================================
    always @(posedge clk or negedge resetn) begin
        if (!resetn || clear_quire) begin
            ...
            accum_reg1 <= accum_reg1 + prod1;
            accum_reg2 <= accum_reg2 + prod2;
            accum_reg3 <= accum_reg3 + prod3;
