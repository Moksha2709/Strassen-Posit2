// =============================================================================
// posit_mxu.v — Verilog (Delayed Normalization Architecture with 2D BRAM Arrays)
...
// 2D Memory Array Syntax for True Block RAM (RAMB18/RAMB36) Primitives.
// =============================================================================
...
module posit_mxu #(
    ...
    // True 2D Memory Array Syntax for Block RAM (RAMB18E2/RAMB36E2) Inference
    // =========================================================================
    ...
    generate
        for (gi = 0; gi < SZI; gi = gi + 1) begin : skew_gen
            if (gi == 0) begin : no_delay
                ...
                (* ram_style = "block" *) reg [35:0] delay_mem [0:gi-1];
                integer k;
                always @(posedge clk or negedge resetn) begin
                    if (!resetn) begin
                        ...
                        delay_mem[0] <= a_dec[gi*36 +: 36];
                        for (k = 1; k < gi; k = k + 1)
                            delay_mem[k] <= delay_mem[k-1];
                    end
                end
                assign a_skewed[gi*36 +: 36] = delay_mem[gi-1];
            end
        end
    endgenerate
    ...
    generate
        for (gj = 0; gj < SZJ; gj = gj + 1) begin : skew_gen_b
            if (gj == 0) begin : no_delay_b
                ...
                (* ram_style = "block" *) reg [47:0] delay_mem_b [0:gj-1];
                integer kb;
                always @(posedge clk or negedge resetn) begin
                    if (!resetn) begin
                        for (kb = 0; kb < gj; kb = kb + 1)
                            delay_mem_b[kb] <= 48'b0;
                    end else begin
                        delay_mem_b[0] <= b[gj*48 +: 48];
                        for (kb = 1; kb < gj; kb = kb + 1)
                            delay_mem_b[kb] <= delay_mem_b[kb-1];
                    end
                end
                assign b_skewed[gj*48 +: 48] = delay_mem_b[gj-1];
            end
        end
    endgenerate
