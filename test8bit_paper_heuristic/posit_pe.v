...
// Uses simple 1-way right-shift for scale alignment (eliminates 1,344 bidirectional barrel shifters!).
// =============================================================================
...
module posit_pe (
    ...
    // Data paths (3-channel packed buses)
    input  wire [71:0]                  posit_in_a,    // Activations (from west): 3x 12-bit decoded Posits
    input  wire [95:0]                  posit_in_b,    // Weights/Shift-in (from north): 3x 12-bit decoded Posits
    output wire [71:0]                  posit_out_a,   // Activations (to east)
    output wire [95:0]                  posit_out_b    // Weights/Shift-out (to south)
);

    // Pipeline registers
    reg [71:0] act_reg;
    reg [71:0] weight_reg;
    reg [95:0] readout_reg;
    ...
    // =========================================================================
    // Multipliers Unit — Tri-Product DSP Packing Architecture
...
`endif
    ...
    // 4. Extraction of Products (Pure DSP Packing using Paper Heuristic Rule, No Fabric Soft Logic)
    wire [7:0] P1_exact = {P_internal[5:0], C_final_padding_0[1:0]};
    
    // Paper Eq. 6: Read P2 directly from DSP multiplier output P_internal[23:18] with heuristic correction
    wire [5:0] P2_raw = P_internal[23:18];
    wire [5:0] sum_ch2 = ((w2_msb + a2_msb > 6'd16) || (w2_msb == 3'd4 && a2_msb == 3'd4)) ? (P2_raw - 6'd1) : P2_raw;
    wire [7:0] P2_exact = {sum_ch2, C_final_padding_1[1:0]};
    ...
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            if (load_weight)
                weight_reg <= posit_in_b[71:0];

            if (shift_load)
                readout_reg <= {48'b0, ch3_word, ch2_word, ch1_word};
            else if (shift_out)
    ...
    assign posit_out_b = shift_out ? readout_reg : {24'b0, weight_reg};
