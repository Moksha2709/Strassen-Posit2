...
// Controlled dynamically via command line plusarg `+simd_mode=0` or `+simd_mode=1`.
// =============================================================================
`timescale 1ns/1ps
...
module eval_tb;
    ...
    initial begin
        // Read the dynamic simd_mode plusarg from command line
        if (!$value$plusargs("simd_mode=%d", simd_mode)) begin
            simd_mode = 0; // Default to 8-bit Triple mode
        end
        $display("[TB INFO] Starting simulation with simd_mode = %d", simd_mode);
