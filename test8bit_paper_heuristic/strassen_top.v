...
// Parallel Multisystolic Accelerator Top-Level with Triple-Packed / SIMD Posits
// Computes three parallel 8-bit or six parallel 4-bit matrix multiplications concurrently.
// =============================================================================
...
module strassen_top #(
    ...
    // =========================================================================
    // 5. Early Decoders (Multiplexed 8-bit or 4-bit Posit to 16-bit Q4.4 Fixed-Point)
    // =========================================================================
    wire [SZI*96-1:0] a11_col_fixed, a12_col_fixed, a21_col_fixed, a22_col_fixed;
    wire [SZJ*96-1:0] b11_row_fixed, b12_row_fixed, b21_row_fixed, b22_row_fixed;

    genvar da, ch;
    generate
        for (da = 0; da < SZI; da = da + 1) begin : dec_a_loop
            for (ch = 0; ch < 3; ch = ch + 1) begin : dec_a_ch_8b
                posit_to_fixed_conv_8b d8_a11 (.in(a11_col[(da*24 + ch*8) +: 8]), .out(a11_col_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_a12 (.in(a12_col[(da*24 + ch*8) +: 8]), .out(a12_col_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_a21 (.in(a21_col[(da*24 + ch*8) +: 8]), .out(a21_col_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_a22 (.in(a22_col[(da*24 + ch*8) +: 8]), .out(a22_col_fixed[(da*96 + ch*16) +: 16]));
            end
            assign a11_col_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign a12_col_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign a21_col_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign a22_col_fixed[(da*96 + 48) +: 48] = 48'sd0;
        end

        for (da = 0; da < SZJ; da = da + 1) begin : dec_b_loop
            for (ch = 0; ch < 3; ch = ch + 1) begin : dec_b_ch_8b
                posit_to_fixed_conv_8b d8_b11 (.in(b11_row[(da*24 + ch*8) +: 8]), .out(b11_row_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_b12 (.in(b12_row[(da*24 + ch*8) +: 8]), .out(b12_row_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_b21 (.in(b21_row[(da*24 + ch*8) +: 8]), .out(b21_row_fixed[(da*96 + ch*16) +: 16]));
                posit_to_fixed_conv_8b d8_b22 (.in(b22_row[(da*24 + ch*8) +: 8]), .out(b22_row_fixed[(da*96 + ch*16) +: 16]));
            end
            assign b11_row_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign b12_row_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign b21_row_fixed[(da*96 + 48) +: 48] = 48'sd0;
            assign b22_row_fixed[(da*96 + 48) +: 48] = 48'sd0;
        end
    endgenerate

    // =========================================================================
    // 6. Preprocessing (Fixed-Point additions, 96-bit wide)
    // =========================================================================
    wire [SZI*96-1:0] prep_a_out_fixed [1:7];

    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst1 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(a11_col_fixed), .in_b(a22_col_fixed), .out(prep_a_out_fixed[1]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst2 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(a21_col_fixed), .in_b(a22_col_fixed), .out(prep_a_out_fixed[2]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst3 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b1), .in_a(a11_col_fixed), .in_b({SZI*96{1'b0}}), .out(prep_a_out_fixed[3]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst4 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b1), .in_a(a22_col_fixed), .in_b({SZI*96{1'b0}}), .out(prep_a_out_fixed[4]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst5 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(a11_col_fixed), .in_b(a12_col_fixed), .out(prep_a_out_fixed[5]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst6 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(a21_col_fixed), .in_b(a11_col_fixed), .out(prep_a_out_fixed[6]));
    strassen_preprocess #(.WIDTH(SZI)) prep_a_inst7 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(a12_col_fixed), .in_b(a22_col_fixed), .out(prep_a_out_fixed[7]));

    wire [SZJ*96-1:0] prep_b_out_fixed [1:7];

    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst1 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(b11_row_fixed), .in_b(b22_row_fixed), .out(prep_b_out_fixed[1]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst2 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b1), .in_a(b11_row_fixed), .in_b({SZJ*96{1'b0}}), .out(prep_b_out_fixed[2]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst3 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(b12_row_fixed), .in_b(b22_row_fixed), .out(prep_b_out_fixed[3]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst4 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(b21_row_fixed), .in_b(b11_row_fixed), .out(prep_b_out_fixed[4]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst5 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b1), .in_a(b22_row_fixed), .in_b({SZJ*96{1'b0}}), .out(prep_b_out_fixed[5]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst6 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(b11_row_fixed), .in_b(b12_row_fixed), .out(prep_b_out_fixed[6]));
    strassen_preprocess #(.WIDTH(SZJ)) prep_b_inst7 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(b21_row_fixed), .in_b(b22_row_fixed), .out(prep_b_out_fixed[7]));

    // =========================================================================
    // 7. Instantiate 7 Parallel Matrix Execution Units (MXUs)
    // =========================================================================
    wire [SZJ*96-1:0] sys_q_out_fixed [1:7];
    ...
    // =========================================================================
    // 8. Postprocessing Stage 1 (Fixed-Point, 96-bit wide)
    // =========================================================================
    wire [SZJ*96-1:0] post1_out_fixed [1:6];

    strassen_preprocess #(.WIDTH(SZJ)) post1_inst1 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(sys_q_out_fixed[1]), .in_b(sys_q_out_fixed[4]), .out(post1_out_fixed[1]));
    strassen_preprocess #(.WIDTH(SZJ)) post1_inst2 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(sys_q_out_fixed[7]), .in_b(sys_q_out_fixed[5]), .out(post1_out_fixed[2]));
    strassen_preprocess #(.WIDTH(SZJ)) post1_inst3 (.clk(clk), .resetn(resetn), .op_sub(1'b1), .passthrough(1'b0), .in_a(sys_q_out_fixed[1]), .in_b(sys_q_out_fixed[2]), .out(post1_out_fixed[3]));
    strassen_preprocess #(.WIDTH(SZJ)) post1_inst4 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(sys_q_out_fixed[3]), .in_b(sys_q_out_fixed[6]), .out(post1_out_fixed[4]));
    strassen_preprocess #(.WIDTH(SZJ)) post1_inst5 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(sys_q_out_fixed[3]), .in_b(sys_q_out_fixed[5]), .out(post1_out_fixed[5]));
    strassen_preprocess #(.WIDTH(SZJ)) post1_inst6 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(sys_q_out_fixed[2]), .in_b(sys_q_out_fixed[4]), .out(post1_out_fixed[6]));

    // Align delays (4-stage delay pipeline)
    reg [SZJ*96-1:0] c12_delay_fixed [0:3];
    reg [SZJ*96-1:0] c21_delay_fixed [0:3];
    integer d_idx;
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            for (d_idx = 0; d_idx < 4; d_idx = d_idx + 1) begin
                c12_delay_fixed[d_idx] <= {(SZJ*96){1'b0}};
                c21_delay_fixed[d_idx] <= {(SZJ*96){1'b0}};
            end
        end else begin
    ...
    wire [SZJ*96-1:0] c12_aligned_fixed = c12_delay_fixed[3];
    wire [SZJ*96-1:0] c21_aligned_fixed = c21_delay_fixed[3];

    // =========================================================================
    // 9. Postprocessing Stage 2 (Fixed-Point, 96-bit wide)
    // =========================================================================
    wire [SZJ*96-1:0] c11_stage2_fixed;
    wire [SZJ*96-1:0] c22_stage2_fixed;

    strassen_preprocess #(.WIDTH(SZJ)) post2_inst1 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(post1_out_fixed[1]), .in_b(post1_out_fixed[2]), .out(c11_stage2_fixed));
    strassen_preprocess #(.WIDTH(SZJ)) post2_inst2 (.clk(clk), .resetn(resetn), .op_sub(1'b0), .passthrough(1'b0), .in_a(post1_out_fixed[3]), .in_b(post1_out_fixed[4]), .out(c22_stage2_fixed));

    // =========================================================================
    // 10. Late Encoders (Q4.4 Fixed-Point to 8-bit/4-bit Posit)
    // =========================================================================
    ...
    generate
        for (ec = 0; ec < SZJ; ec = ec + 1) begin : enc_post_gen
            for (ech = 0; ech < 3; ech = ech + 1) begin : enc_post_ch_8b
                fixed_to_posit_conv_8b e8_c11 (.in(c11_stage2_fixed[(ec*96 + ech*16) +: 16]), .out(c11_stage2[(ec*24 + ech*8) +: 8]));
                fixed_to_posit_conv_8b e8_c12 (.in(c12_aligned_fixed[(ec*96 + ech*16) +: 16]), .out(c12_aligned[(ec*24 + ech*8) +: 8]));
                fixed_to_posit_conv_8b e8_c21 (.in(c21_aligned_fixed[(ec*96 + ech*16) +: 16]), .out(c21_aligned[(ec*24 + ech*8) +: 8]));
                fixed_to_posit_conv_8b e8_c22 (.in(c22_stage2_fixed[(ec*96 + ech*16) +: 16]), .out(c22_stage2[(ec*24 + ech*8) +: 8]));
            end
        end
    endgenerate
    ...
    // =========================================================================
    // 11. Stream Output Interface (3x 8b or 6x 4b packed matrix results)
    // =========================================================================
