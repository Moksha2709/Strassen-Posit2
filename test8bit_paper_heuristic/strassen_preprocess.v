...
// Parallel 6-Channel 16-bit Fixed-Point Adders for Pre/Postprocessing
...
module strassen_preprocess #(
    ...
    // Stream data paths (flat: carrying 6 packed 16-bit fixed elements per index)
    input  wire [WIDTH*96-1:0]              in_a,
    input  wire [WIDTH*96-1:0]              in_b,
    output wire [WIDTH*96-1:0]              out
);

    genvar gj, ch;
    generate
        for (gj = 0; gj < WIDTH; gj = gj + 1) begin : element_loop
            for (ch = 0; ch < 6; ch = ch + 1) begin : channel_loop
                wire [15:0] a_val = in_a[(gj*96 + ch*16) +: 16];
                wire [15:0] b_val = passthrough ? 16'b0 : in_b[(gj*96 + ch*16) +: 16];
