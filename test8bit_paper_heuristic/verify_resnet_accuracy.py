...
def tiled_gemm_hw(matrix_a, matrix_b, M, K, N, use_relu=True):
    # Per-layer weight scale calibration (scales weights to fill Posit8 range, avoiding underflow)
    ...
    for mt in range(n_mt):
        for nt in range(n_nt):
            ...
            for kt in range(n_kt):
                ...
                for i in range(TILE):
                    for j in range(TILE):
                        acc[i][j] += (hw_tile[i][j] / w_scale)  # De-scale partial sums
            for i in range(TILE):
                for j in range(TILE):
                    v = acc[i][j]
...
# -----------------------------------------------------------------------------
def main(layer_indices=None):
    print("=" * 80)
    print("  ResNet-50 Real-Layer Accuracy Validation: 8-Bit Triple-Packed Posit DLA")
    print("=" * 80)

    layers = get_resnet50_conv_layers(pretrained=True)
    if layer_indices is None:
        # Stop after Layer 0 (first conv7x7) as requested
        layer_indices = [0]

    for idx in layer_indices:
        spec = layers[idx]
        inp, wt = spec["input"], spec["weight"]
        inp = inp[:, :16, :16]  # Fast 16x16 spatial patch for instant hardware simulation
        C, H, W = inp.shape
        O, _, kh, kw = wt.shape
        ...
        print(f"\nLayer [{idx}] {spec['name']}: M={M} K={K} N={N} "
              f"({math.ceil(M/TILE)}x{math.ceil(K/TILE)}x{math.ceil(N/TILE)} tiles)")
