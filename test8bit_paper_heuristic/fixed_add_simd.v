...
module fixed_add_simd (
    input  wire         clk,
    input  wire         resetn,
    input  wire         simd_mode, // 0 = 12-bit mode, 1 = dual 6-bit SIMD
    ...
    always @(*) begin
        if (simd_mode) begin
            // Dual 16-bit packed fixed-point additions (Q4.4 format in each 16-bit slice)
            if (op_sub) begin
                sum_comb[31:16] = in_a[31:16] - in_b[31:16];
