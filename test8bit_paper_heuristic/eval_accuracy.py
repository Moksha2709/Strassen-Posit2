...
def run_sim(simd_mode):
    print(f"[INFO] Running simulation with vvp (simd_mode = {simd_mode})...")
    subprocess.run(["vvp", "eval_sim.vvp", f"+simd_mode={simd_mode}"], check=True)
...
def main():
    ...
    # =========================================================================
    # PART 2: VERIFY 4-BIT SIMD MODE (6 channels)
    # =========================================================================
    print("\n" + "="*80)
    print("             PART 2: VERIFYING 6-WAY 4-BIT SIMD MODE")
    print("="*80)

    A_4b = [[[random.normalvariate(0, 0.5) for _ in range(16)] for _ in range(16)] for _ in range(6)]
    B_4b = [[[random.normalvariate(0, 0.5) for _ in range(16)] for _ in range(16)] for _ in range(6)]

    quads_A_4b = [get_quadrants(A_4b[i]) for i in range(6)]
    quads_B_4b = [get_quadrants(B_4b[i]) for i in range(6)]

    with open("input_a.txt", "w") as f:
        for tile in range(4):
            for r in range(8):
                rows = [quads_A_4b[c][tile][r] for c in range(6)]
                f.write(pack_hex_row_4b(rows) + "\n")
    with open("input_b.txt", "w") as f:
        for tile in range(4):
            for r in range(8):
                rows = [quads_B_4b[c][tile][r] for c in range(6)]
                f.write(pack_hex_row_4b(rows) + "\n")

    run_sim(1) # simd_mode = 1

    with open("output_c.txt", "r") as f:
        lines = [line.strip() for line in f.readlines() if line.strip()]

    C11_4b = [[] for _ in range(6)]
    C12_4b = [[] for _ in range(6)]
    C21_4b = [[] for _ in range(6)]
    C22_4b = [[] for _ in range(6)]

    for r in range(8):
        cols11 = parse_hex_row_4b(lines[r])
        cols12 = parse_hex_row_4b(lines[r+8])
        cols21 = parse_hex_row_4b(lines[r+16])
        cols22 = parse_hex_row_4b(lines[r+24])
        for c in range(6):
            C11_4b[c].append(cols11[c])
            C12_4b[c].append(cols12[c])
            C21_4b[c].append(cols21[c])
            C22_4b[c].append(cols22[c])

    C_hw_4b = []
    for c in range(6):
        M = []
        for i in range(8): M.append(C11_4b[c][i] + C12_4b[c][i])
        for i in range(8): M.append(C21_4b[c][i] + C22_4b[c][i])
        C_hw_4b.append(M)

    C_gt_4b = [matmul(A_4b[i], B_4b[i]) for i in range(6)]

    sqnr_4b = []
    for i in range(6):
        sq, ms = calc_sqnr(C_hw_4b[i], C_gt_4b[i])
        sqnr_4b.append(sq)
        print(f"Channel {i+1} (4-bit SIMD) SQNR: {sq:.2f} dB  | MSE: {ms:.3e}")
    ...
    print(f"6-Way 4-Bit SIMD Mode Average SQNR  : {sum(sqnr_4b)/6.0:.2f} dB")
    print("-"*80)
    print("Verified Execution Correctness for 9 Concurrent Matrix GEMM Tasks!")
    print("="*80)
