...
def encode_fixed8(x):
    """Encodes float32 into signed 8-bit fixed-point (Q4.4 format)."""
...
def compute_q44_gemm(matrix_a, matrix_b, M, K, N, use_relu=True):
    c_out = [[0.0] * N for _ in range(M)]
    for i in range(M):
        for j in range(N):
            acc = 0.0
            for k in range(K):
                a_q = decode_fixed8(encode_fixed8(matrix_a[i][k]))
                b_q = decode_fixed8(encode_fixed8(matrix_b[k][j]))
                acc += a_q * b_q
...
def main(weights="../test8bit/weights_resnet50_cifar10_seed42_fp32.pth", dataset="cifar10", layer_indices=None):
    print("=" * 80)
    print("  ResNet-50 Multi-Layer Hardware Validation: 8-Bit Fixed-Point (Q4.4) Baseline")
    print("=" * 80)
    ...
    for idx in layer_indices:
        ...
        hw_c = compute_q44_gemm(matrix_a, matrix_b, M, K, N, use_relu=True)
    ...
    print("\n" + "=" * 80)
    print("Verified Status   : SUCCESS (8-Bit Fixed-Point Q4.4 Baseline Validation)")
    print("=" * 80 + "\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="8-Bit Fixed Baseline Hardware Validation (Q4.4)")
