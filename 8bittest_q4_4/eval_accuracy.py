...
def float_to_hex_row(row_vals):
    fixed_words = [encode_fixed8(x) for x in row_vals]
    hex_str = "".join(f"{val:02x}" for val in reversed(fixed_words))
    return hex_str
...
def main():
    ...
    print("="*65)
    print(f"{'Metric':<25} | {'Reference GEMM (INT8)':<20} | {'Our 8-bit Fixed Q4.4':<20}")
    print("-"*65)
