...
wire [Bs:0] lr_N;
wire [Bs:0] sr_N;
abs_regime_add #(.N(Bs)) uut_abs_regime1 (lrc, lr, lr_N);
abs_regime_add #(.N(Bs)) uut_abs_regime2 (src, sr, sr_N);
...
wire [N:0] add_m;
add_sub_N_add #(.N(N)) uut_add_sub_N (op, add_m_in1, DSR_right_out, add_m);
...
add_1_add #(.N(es+Bs+1)) uut_add_mantovf (le_o_tmp, mant_ovf[1], le_o);
...
wire [N:0] ain = {1'b0,a};
wire [N:0] bin = {1'b0,b};
add_N_in_add #(.N(N)) a1 (ain,bin,c);
endmodule

/////////////////////////
module sub_N_in_add (a,b,c);
parameter N=10;
input [N:0] a,b;
output [N:0] c;
assign c = a - b;
