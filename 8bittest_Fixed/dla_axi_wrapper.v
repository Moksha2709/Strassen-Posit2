...
// Configured for 64-bit SRAM widths (8-bit fixed-point baseline)
// =============================================================================
...
module dla_axi_wrapper #(
    ...
    // Master DRAM Memory Port Interface (64-bit)
    output wire                             dram_rd_en,
    output wire [31:0]                      dram_rd_addr,
    input  wire [63:0]                      dram_rd_data,

    output wire                             dram_wr_en,
    output wire [31:0]                      dram_wr_addr,
    output wire [63:0]                      dram_wr_data
);

    // Register Map offsets
    localparam [5:0]
        REG_CTRL         = 6'h00, // [0]: Start, [8]: Ready, [9]: Done
        ...
        REG_SRAM_DATA_1  = 6'h14, // Data bits [63:32] (Triggers write transaction)
    ...
    wire [63:0] weight_wr_data_cpu = {s_axi_wdata, reg_sram_data_0};

    reg         act_wr_en_cpu;
    wire [6:0]  act_wr_addr_cpu    = reg_sram_addr[6:0];
    wire [63:0] act_wr_data_cpu    = {s_axi_wdata, reg_sram_data_0};
    reg         act_rd_en_cpu;
    wire [6:0]  act_rd_addr_cpu    = reg_sram_addr[6:0];
    wire [63:0] act_rd_data;
    ...
    wire [63:0]  final_weight_wr_data = (dma_busy) ? dma_sram_wr_data : weight_wr_data_cpu;
    ...
    wire [63:0]  final_act_wr_data    = (dma_busy) ? dma_sram_wr_data : act_wr_data_cpu;
                ...
                WSTATE_DATA: begin
                    if (s_axi_wvalid && s_axi_wready) begin
                        ...
                        case (waddr)
                            ...
                            REG_SRAM_DATA_1:begin
                                reg_sram_data_1 <= s_axi_wdata;
