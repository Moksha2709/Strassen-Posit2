...
// DLA controller supporting OP_MATMUL and OP_VADD slot execution
// Configured for 64-bit widths (8-bit fixed-point baseline)
// =============================================================================
...
module dla_controller (
    ...
    input  wire [63:0]                      weight_rd_data,
    ...
    output wire [63:0]                      input_a_data,
    input  wire                             input_a_rdy,
    output wire                             input_b_val,
    output wire [63:0]                      input_b_data,
    ...
    // 4-cycle pipeline delay shift register for vector addition
    // (strassen_preprocess has 4-cycle latency for fixed-point addition)
    reg [3:0] vadd_val_pipe;
    ...
    // Pipeline delay shift register to handle 4-cycle fixed-point addition delay
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            vadd_val_pipe <= 4'b0;
        end else begin
            if (state == STATE_WAIT && inst_op_reg == OP_VADD) begin
                vadd_val_pipe <= {vadd_val_pipe[2:0], (read_cnt > 6'd0 && read_cnt <= 6'd32 && act_rd_en)};
            end else begin
                vadd_val_pipe <= 4'b0;
            end
        end
    ...
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            if (state == STATE_WAIT) begin
                ...
                end else if (inst_op_reg == OP_VADD && vadd_val_pipe[3]) begin
                    write_cnt <= write_cnt + 6'd1;
                end
            end else begin
                write_cnt <= 6'd0;
    ...
    assign act_wr_en   = (state == STATE_WAIT) ? 
                         ((inst_op_reg == OP_MATMUL) ? (out_val && out_rdy) : vadd_val_pipe[3]) : 
                         1'b0;
